...
func Parse(request []byte) (login.IdentityData, login.ClientData, login.AuthResult, error) {
	var (
		iData login.IdentityData
		cData login.ClientData
		res   login.AuthResult
		key   = &ecdsa.PublicKey{}
	)
	...
	return identityClaims.ExtraData, cData, login.AuthResult{PublicKey: key, XBOXLiveAuthenticated: authenticated}, nil
}
...
func Encode(loginChain string, multiplayerToken string, data login.ClientData, key *ecdsa.PrivateKey, legacy bool) []byte {
...
func EncodeOffline(identityData login.IdentityData, data login.ClientData, key *ecdsa.PrivateKey, legacy bool) []byte {
	keyData := MarshalPublicKey(&key.PublicKey)
	claims := jwt.Claims{
		Expiry:    jwt.NewNumericDate(time.Now().Add(time.Hour * 6)),
		NotBefore: jwt.NewNumericDate(time.Now().Add(-time.Hour * 6)),
